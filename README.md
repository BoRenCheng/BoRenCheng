# Hi there, I'm Bo-Ren Cheng (鄭博仁) 

**Computer Science Junior @ Soochow University (SCU)** **AI Researcher | FinTech Developer | Data Science Enthusiast**

目前就讀於東吳大學資訊科學系三年級。我熱衷於透過數據科學與機器學習解決複雜問題，研究領域涵蓋 **AI 治理成熟度評估**、**量化投資模型開發** 以及 **第三方腳本安全稽核**。我致力於將學術研究轉化為具備實務價值的技術方案。

---

## 🚀 核心亮點 (Highlights)

* **國際學術發表**：於 **ICBDA 2026 (東京早稻田大學)** 發表全英文論文並進行口頭報告，研究重點為 Robots.txt 治理策略之大數據分析。
* **量化投資實證**：開發台股多因子選股模型（1999-2025），實作 OLS、隨機森林與深度神經網路（NN1-NN5），在特定分組下達成 **61.9% 勝率** 與優於大盤 **2.5 倍之夏普比率**。
* **創新創業經歷**：曾參與教育部 **U-start 創新創業計畫**（FinancePT）並在多項資料科學與 FinTech 競賽中獲獎（中研院資料科學競賽優選、法國巴黎人壽創新競賽佳作）。

---

## 🛠 技術棧 (Tech Stack)

* **Languages:** Python (Primary), SQL, JavaScript
* **Data Science:** Pandas, NumPy, Scikit-Learn, TensorFlow, Keras, XGBoost
* **Web Scraping & Tools:** BeautifulSoup4, Requests, Line Messaging API, Google Maps API
* **Deployment:** Render, Docker, Git

---

## 📚 精選專案 (Featured Projects)

### 📈 [FinTech-ML-Factor-Investing](https://github.com/BoRenCheng/FinTech-ML-Factor-Investing)
運用機器學習與深度學習預測財務因子 IC 值，並進行 26 年台股長週期回測的量化投資框架。

### 🤖 [LineBot-InfoGrabber](https://github.com/BoRenCheng/LineBot-InfoGrabber)
整合氣象署 API、環境部資料與即時新聞爬蟲的適地性生活助理，具備地震即時示警功能。

### 🛡️ [Third-party Script Governance Auditor](連結待補)
量化稽核網站第三方腳本依賴性的治理工具，提升網頁資源透明度與風險管理。

---

## 🏆 榮譽與認證 (Honors & Certifications)

* **中研院資料科學競賽** - 優選 (高齡族群長照分析)
* **NVIDIA 認證** - Fundamentals of Deep Learning / Omniverse Extension Development
* **Google Analytics** - GA4 專業認證
* **TOEIC** - 745 分

---

## 📬 聯絡我 (Connect with me)

* **LinkedIn:** [Your LinkedIn Profile]
* **Email:** [Your Email Address]
* **Location:** Taipei, Taiwan

---
"The best way to predict the future is to create it."
<!--
**BoRenCheng/BoRenCheng** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
